...
```{r hiddenPrep, echo=FALSE, results='hide'}
# Set language to English for consistency
Sys.setlocale("LC_TIME", "C")

# Set working directory
setwd("C:/Users/spo12/Dropbox/coursera/Reproducible Research/RepData_PeerAssessment1")
```
...
```{r Preparations, echo=TRUE, warning=FALSE, message=FALSE}
# Load libraries
library(plyr)
library(ggplot2)
library(Hmisc)
...
```{r PerDayAv, echo=TRUE, results='hold'}
summary(total_PD)
...
In plain text: the mean number of steps taken per day is `r format(mean_PD, nsmall=2, big.mark=",")`, while the median is `r format(median_PD, nsmall=2, big.mark=",")`.

## What is the average daily activity pattern?

Now that we know that the person whose data we are analysing took around `r format(mean_PD, nsmall=2, big.mark=",")` steps per day, we would also like to know how these steps were distributed over an average day. To analyse this, we're calculating the mean number of steps for each five-minute interval measured, across all days.
...
The interval with the most steps (as a daily average) is that between 8:35 and 8:40 a.m., and the average number of steps there is `r format(max(total_TS$mean_steps), nsmall=2, big.mark=",")`.
...
```{r ImpPerDayAv, echo=TRUE, results='hold'}
...
For the mean and media, though, the imputation of missing values brought along big changes. While the old mean was `r format(mean_PD, nsmall=2, big.mark=",")`, it is now `r format(mean_imp, nsmall=2, big.mark=",")`. The median changed from `r format(median_PD, nsmall=2, big.mark=",")` to `r format(median_imp, nsmall=2, big.mark=",")`. The imputation of the missing values therefore helped to assimilate the mean and the median.
...
Indeed, the sum of average steps per day on a weekday is `r format(total_TSW_sum[total_TSW_sum$week=="weekday",]$V1, nsmall=2, big.mark=",")`, while on the weekend our subject typically takes around `r format(total_TSW_sum[total_TSW_sum$week=="weekend",]$V1, nsmall=2, big.mark=",")` steps a day.